...
class DiscordMusicBot extends Client {
  ...
  constructor(
    ...
    ConfigFetcher().then((conf) => {
      ...
      this.LoadEvents();
      setTimeout(() => {
        this.log("Start build...".green);
        this.build();
        this.log("Build success".green);
      }, 5000);
    });
  ...
  build() {
    ...
    this.warn(`Logging in with token: ${this.config.token}`);
    this.login(this.config.token)
        .then(() => this.log("Logged in Discord"))
        .catch((err) => this.error(`Failed to login: ${err}`));
